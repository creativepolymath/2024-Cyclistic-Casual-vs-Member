...
<div class="info">
  ...
  * ggplot for visuals
  * fontawesome for pretty notebook
</div>
...
all_trips_v2 %>%
  filter(!is.na(gender)) %>%
    filter(birthyear >= 1950L & birthyear <= 2003L & !is.na(birthyear)) %>%
      ggplot() +
      aes(x = birthyear, colour = gender) +
      geom_histogram(bins = 20L, color = "white", fill = "#6da5b3") +
      facet_wrap(vars(gender, member_casual)) +
          scale_x_continuous(name="Birth Year") +
          scale_y_continuous(name="Number of Rides") +
          labs(title = "Gender and Age Divisiion of Riders")
```
